...
var json_file = "/reviews_books_first_100000.json"
println("Reading from file: " + json_file);

// Create the DataFrame
val df = sqlContext.read.json(json_file)

val parsedData = df.map(t => Vectors.dense(t.getAs[Double]("overall"), t.getAs[String]("reviewText").length().toDouble, t.getAs[Long]("unixReviewTime").toDouble)).cache()
